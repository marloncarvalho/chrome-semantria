 ...
 * Main Session class
 */
function Session(consumerKey, consumerSecret, serializer, applicationName, useCompression) {
    ...
    this.acceptEncoding = useCompression ? 'gzip, deflate' : 'identity';
    //AppName for passing to the server
    this.applicationName += this.wrapperName + '/' + this.wrapperVersion + '/' + this.format;

	this.host = 'https://api30.semantria.com';
}
...
Session.prototype.runRequest = function(method, url, postData, deserializeElements) {
	var request = new this.AuthRequest(this.consumerKey, this.consumerSecret, this.applicationName, this.acceptEncoding);
...
Session.prototype.addConfigurations = function(proxy) {
    var url = this.host + '/configurations.' + this.format;
    return this.runRequest("POST", url, this.serializer.serialize(proxy));
};
Session.prototype.updateConfigurations = function(proxy) {
    var url = this.host + '/configurations.' + this.format;
    return this.runRequest("POST", url, this.serializer.serialize(proxy));
};
Session.prototype.removeConfigurations = function(proxy) {
	var url = this.host + '/configurations.' + this.format;
	return this.runRequest("DELETE", url, this.serializer.serialize(proxy));
};
    //Blacklist calls
Session.prototype.getBlacklist = function(configId) {
    var url = this.host + '/blacklist.' + this.format;
    if (configId) {
        url += '?config_id=' + configId;
    }
	return this.runRequest("GET", url, null, ["blacklist"]);
};
Session.prototype.addBlacklist = function(proxy, configId) {
    var url = this.host + '/blacklist.' + this.format;
    if (configId) {
        url += '?config_id=' + configId;
    }
...
Session.prototype.removeBlacklist = function(proxy, configId) {
	var url = this.host + '/blacklist.' + this.format;
	if (configId) {
		url += '?config_id=' + configId;
	}
...
Session.prototype.getCategories = function(configId) {
	var url = this.host + '/categories.' + this.format;
	if (configId) {
		url += '?config_id=' + configId;
	}
	return this.runRequest("GET", url, null, ["categories", "samples"]);
};
Session.prototype.addCategories = function(proxy, configId) {
    var url = this.host + '/categories.' + this.format;
    if (configId) {
        url += '?config_id=' + configId;
    }
...
Session.prototype.removeCategories = function(proxy, configId) {
	var url = this.host + '/categories.' + this.format;
	if (configId) {
		url += '?config_id=' + configId;
	}
...
Session.prototype.getQueries = function(configId) {
	var url = this.host + '/queries.' + this.format;
	if (configId) {
		url += '?config_id=' + configId;
	}
	return this.runRequest("GET", url, null, ["queries"]);
};
Session.prototype.addQueries = function(proxy, configId) {
    var url = this.host + '/queries.' + this.format;
    if (configId) {
        url += '?config_id=' + configId;
    }
...
Session.prototype.removeQueries = function(proxy, configId) {
	var url = this.host + '/queries.' + this.format;
	if (configId) {
		url += '?config_id=' + configId;
	}
...
Session.prototype.getEntities = function(configId) {
    var url = this.host + '/entities.' + this.format;
    if (configId) {
        url += '?config_id=' + configId;
    }
    return this.runRequest("GET", url, null, ["entities"]);
};
Session.prototype.addEntities = function(proxy, configId) {
    var url = this.host + '/entities.' + this.format;
    if (configId) {
        url += '?config_id=' + configId;
    }
...
Session.prototype.removeEntities = function(proxy, configId) {
    var url = this.host + '/entities.' + this.format;
    if (configId) {
        url += '?config_id=' + configId;
    }
    ...
    //Phrases calls
Session.prototype.getPhrases = function(configId) {
	var url = this.host + '/phrases.' + this.format;
	if (configId) {
		url += '?config_id=' + configId;
	}
	return this.runRequest("GET", url, null, ["phrases"]);
};
Session.prototype.addPhrases = function(proxy, configId) {
    var url = this.host + '/phrases.' + this.format;
    if (configId) {
        url += '?config_id=' + configId;
    }
...
Session.prototype.updatePhrases = function(proxy, configId) {
    var url = this.host + '/phrases.' + this.format;
    if (configId) {
        url += '?config_id=' + configId;
    }
...
Session.prototype.removePhrases = function(proxy, configId) {
	var url = this.host + '/phrases.' + this.format;
	if (configId) {
		url += '?config_id=' + configId;
	}
...
Session.prototype.queueDocument = function(doc, configId) {
	var url = this.host + '/document.' + this.format;
	if (configId) {
		url += '?config_id=' + configId;
	}
...
Session.prototype.queueBatchOfDocuments = function(batch, configId) {
	var url = this.host + '/document/batch.' + this.format;
	if (configId) {
		url += '?config_id=' + configId;
	}

	result = this.runRequest("POST", url,
							this.serializer.serialize(batch),
							['documents', 'themes', 'entities', 'topics', 'phrases']);
    if (result && result.constructor == Array) {
        this.onDocsAutoResponse(result);
        return 200;
...
Session.prototype.queueCollection = function(coll, configId) {
    var url = this.host + '/collection.' + this.format;
    if (configId) {
        url += '?config_id=' + configId;
    }

    result = this.runRequest("POST", url,
                            this.serializer.serialize(coll),
...
Session.prototype.getDocument = function(id, configId) {
	...
	var url = this.host + '/document/' + id + '.' + this.format;
	if (configId) {
		url += '?config_id=' + configId;
	}
...
Session.prototype.getCollection = function(id, configId) {
    ...
    var url = this.host + '/collection/' + id + '.' + this.format;
    if (configId) {
        url += '?config_id=' + configId;
    }
...
Session.prototype.getProcessedDocuments = function(configId) {
	var url = this.host + '/document/processed.' + this.format;
	if (configId) {
		url += '?config_id=' + configId;
	}
	return this.runRequest("GET", url, null, ['documents', 'themes', 'entities', 'topics', 'phrases']);
...
Session.prototype.getProcessedCollections = function(configId) {
	var url = this.host + '/collection/processed.' + this.format;
	if (configId) {
		url += '?config_id=' + configId;
	}
	return this.runRequest("GET", url, null, ['collections', 'themes', 'entities', 'topics', 'facets', 'attributes']);
 ...
 * AuthRequest class
 */
Session.prototype.AuthRequest = function(consumerKey, consumerSecret, applicationName, acceptEncoding) {
